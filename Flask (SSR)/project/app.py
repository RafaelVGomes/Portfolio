...
from helpers import apology, login_required, lookup, usd, percentual_diff
...
db.execute("CREATE TABLE IF NOT EXISTS business (id INTEGER PRIMARY KEY, cash NUMERIC NOT NULL DEFAULT 0);")
if not db.execute("SELECT * FROM business"):
    db.execute("INSERT INTO business (cash) VALUES(0);")

db.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username TEXT NOT NULL, hash TEXT NOT NULL);")

db.execute("CREATE TABLE IF NOT EXISTS items (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, item_name TEXT NOT NULL UNIQUE, amount NUMERIC NOT NULL, price NUMERIC NOT NULL, sale_price NUMERIC, is_product INTEGER NOT NULL, quantity_alert INTEGER NOT NULL, measure TEXT NOT NULL);")
db.execute("CREATE INDEX IF NOT EXISTS idx_items ON items (item_name);")

db.execute("CREATE TABLE IF NOT EXISTS products (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, product_name TEXT NOT NULL UNIQUE, amount NUMERIC NOT NULL, measure TEXT NOT NULL, quantity_alert INTEGER, price NUMERIC NOT NULL, has_recipe INTEGER NOT NULL);")
db.execute("CREATE INDEX IF NOT EXISTS idx_product ON products (product_name);")

db.execute("CREATE TABLE IF NOT EXISTS recipes (product_id INTEGER NOT NULL, item_id INTEGER NOT NULL, item_name TEXT NOT NULL, amount NUMERIC NOT NULL, FOREIGN KEY (product_id) REFERENCES products (id) ON UPDATE CASCADE ON DELETE CASCADE, FOREIGN KEY (item_id) REFERENCES items (id) ON UPDATE CASCADE ON DELETE CASCADE);")
db.execute("CREATE INDEX IF NOT EXISTS idx_recipes ON recipes (product_id);")

db.execute("CREATE TABLE IF NOT EXISTS historic (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, date TEXT DEFAULT CURRENT_TIMESTAMP, trade TEXT NOT NULL, price NUMERIC NOT NULL, amount INTEGER NOT NULL, FOREIGN KEY (user_id) REFERENCES users (id) ON UPDATE CASCADE ON DELETE CASCADE);")
db.execute("CREATE INDEX IF NOT EXISTS idx_historic ON historic (user_id, trade);")
...
def add_item():
    ...
    if request.method == "GET":
        ...
        USER_ID = session['user_id']
        PROD_NAME = request.form.get("item_name").lower()
        if PROD_NAME == "":
            return apology("Please enter a item name")
        ...
        db.execute("UPDATE business SET cash = business.cash - ? WHERE id = 1", CASH)

        return redirect("/add-item")


@app.route("/upd-item/<int:id>", methods=["GET", "POST"])
@login_required
def upd_item(id):
